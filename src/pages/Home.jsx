...
const Home = (props) => {
    ...
    if (validate(nick, 5, 'more') && validate(port, 4, 'equal')) {
      axios.post('/ws', { port: Number.parseInt(port), nickname: nick })
        .then(data => {
          ...
          setIp(address);
          connectDesktopClient(address, (conn) => {
            conn.send(`nickname browser-${nick}`);
            props.plsSetThisClient(conn);
            setDone(1);
          });
          return data.data;
        })
  ...
  const connectClient = () => {
    if (validate(nick, 5, 'more')) {
      axios.put('/ws', { nickname: nick, address: port })
        .then((data) => {
          setIp(port);
          connectDesktopClient(`${port}`, (conn) => {
            conn.send(`nickname browser-${nick}`);
            props.plsSetThisClient(conn);
            setDone(1);
          });
